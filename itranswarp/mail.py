...
class SmtpConfig(object):
    ' SMTP config '
    def __init__(self, host, username=None, password=None, port=25, use_tls=False):
        '''
        Create a SMTP config.

        host: smtp host like 'smtp.example.com'
        username: authentication username, default to None if no authentication needed.
        password: authentication password, default to None if no authentication needed.
        port: smtp port, default to 25.
        use_tls: use TLS, default to False. Set True (and port 587) if using GMail SMTP.
        '''
        self._host = host
...
def send_mail(smtp_cfg, from_addr, to_addrs, subject='', body='', cc_addrs=None, bcc_addrs=None, attaches=None):
    ...
    >>> yahoo = SmtpConfig('smtp.mail.yahoo.com', 'itranswarp@yahoo.com', 'plz-do-not-change')
    ...
    >>> gmail = SmtpConfig('smtp.gmail.com', 'itranswarptest@gmail.com', 'plz-do-not-change', 587, True)
    ...
    server = smtplib.SMTP(smtp_cfg.host)
    if smtp_cfg.use_tls:
        server.starttls()
    u = smtp_cfg.username
    p = smtp_cfg.password
